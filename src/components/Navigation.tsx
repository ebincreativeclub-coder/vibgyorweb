...
export function Navigation() {
  ...
  useMotionValueEvent(scrollY, "change", (latest) => {
    ...
    // Threshold for background change
    setIsScrolled(latest > 50);

    // Only hide after scrolling past the hero section (e.g. 500px)
    if (latest > 500) {
  ...
  // Fade out slightly during the mid-zoom phase of the hero if needed
  const navOpacity = useTransform(scrollY, [100, 300, 1500, 2000], [1, 0.4, 0.4, 1]);

  return (
    <motion.nav 
      animate={{ 
        y: isVisible ? 0 : -100,
        opacity: isScrolled ? 1 : 1 // Maintaining full visibility by default
      }}
      transition={{ 
        duration: 0.4,
        ease: [0.32, 0.72, 0, 1]
    ...
    >
      <div className="relative w-full max-w-[1280px] h-[60px] mx-auto flex items-center px-8">
        
        {/* Logo (Group 310) */}
        <Link href="/" className="absolute left-[88px] top-[10px]">
        ...
        {/* Navigation Links (Home to Contact) */}
        <div className="absolute right-[445px] flex items-center gap-[30px] lg:gap-[40px]">
        ...
        {/* Call Now Button (Rectangle 164) */}
        <Link 
          href="tel:+1234567890" 
          className="absolute right-[86px] top-[12px] flex items-center justify-center bg-[#03AEF2] text-white font-semibold rounded-full shadow-lg transition-all hover:opacity-90 active:scale-95"
          style={{ 
            width: '138px', 
            height: '35px',
            fontSize: '12px',
